...
static IPPAttribute *build_IPPAttribute(ipp_attribute_t * attr)
{
    ...
    if(valTag == IPP_TAG_ZERO ||
	    valTag == IPP_TAG_NOVALUE ||
	    valTag == IPP_TAG_NOTSETTABLE ||
	    valTag == IPP_TAG_ADMINDEFINE) {
...
cupsipp_iocb_read(void* ctx, ipp_uchar_t * buffer, size_t len)
{
    ...
    if (gotbuffer) memcpy(buffer, gotbuffer, got);
