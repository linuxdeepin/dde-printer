...
static int do_model_compare(const char * a, const char * b)
{
    ...
    while ((quick_a = *a) != L'\0' && (quick_b = *b) != L'\0') {
        ...
        if (!cmp) {
            if (end_a != end_b) return end_a < end_b ? -1 : 1;
...
{
    if (uppervalue) *uppervalue = attr->values[element].range.upper;

    return (attr->values[element].range.lower);
}

int
ippGetResolution(ipp_attribute_t * attr,
         int element, int *yres, ipp_res_t * units)
{
    if (yres) *yres = attr->values[element].resolution.yres;

    if (units) *units = attr->values[element].resolution.units;
